...
For some research area, it is essential to access Big data sets to reproduce the scientific computing workflow.  For such cases, it is essential to reproduce the work, using infrastructure have optimised access to the data, so that one does not consume unessesary computing and network resources.  
...
This effort will brididge the gap between binderhub.org to "Home"; users could not control the infrastreucture where they reproduce , to make computational experiments, by giving the freedome to the researchers to chose where they make their computational experiments in a simple manner.   
