...
workflow tool or environment that caters for the given use case (for example
~\cite{reana2019} \TODO{cite other workflow systems}). Or (b) the use standard
(software engineering) computing tools and conventions (git, make, python, perl,
bash, \ldots) to specify the compute environment and automate reproduction
computation.

The workflow tool approach (a) has limitations in that the tool will not support
all use cases, and there may be a vendor lock-in. The standard computing tools
approach (b) is generic but not accessible to all scientists as it requires
sufficient training or experience to be effective.
