...
\item The work done is this project will be \textbf{Supporting} scientists in their
  endeavours to make their work more reproducible and re-usable.
\item We believe in the value of \textbf{Open} science and \textbf{Open} source. The
...
\item We illustrated the reproducibility discussion with
  \fullref{sec:reproducibility-example} and highlighted the current shortcomings\fullref{sec:reproducibility-challenges}.
    We provide details on some of our
  \fullref{sec:science-applications}, how they will contribute to feedback the developments of the 
  Binder tools and serve to showcase the outcomes of \TheProject.  
\item Section~\fullref{sec:community-engagement-panel} presents our strategy to engage with communities, 
  learn from each others and create training materials that are targeting different audiences and 
  contribute to the elaboration of best practices.   
...
We will actively work with representatives of key user communities, from Biophysics, 
Climate change and biodiversity, Molecular Dynamics.
...
\item Reproducibility of analysis pipelines and their software
  ...
  highlithing the ability of the newly developed Binder tools to re-create valid computational environments,
  and re-run the same pipelones, long after their publications (\taskref{applications}{data-publishing}). 
...
\item Reproducibility of Earth System Models with the Norwegian Earth System Modelling Consortium (NCC),
  ...
  The current bottleneck is the need to create a container for each specific simulation (which is not easy to do for a researcher) and
...
and supported by Sigma2\footnote{https://www.sigma2.no/}, the Norwegian National provider of e-infrastructure for the provision of computing, storage and services (Binder deployment planned within \TheProject).

\end{itemize}

\begin{figure}
  \includegraphics[height=4cm]{images/fish.png}
  \includegraphics[height=4cm]{images/octopus-benzene.pdf}
  \includegraphics[height=4cm]{images/gg-a.png}
...
\noindent\textbf{\repotodocker}\label{sec:repo2docker}
is a tool that aims to \myemph{automate existing practices} for reproducible environment specification.
...
\noindent \textbf{BinderHub}\label{sec:binderhub}
is software for hosting a web service built on \repotodocker{} and
