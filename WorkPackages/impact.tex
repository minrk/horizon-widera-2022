...
\begin{wpobjectives}
  ...
  reproducibility to broaden their applicability and increase impact.
  \WPref{applications} exploits these technological advances
  in real-world use cases.

  The objectives of this work package are to
 \begin{compactitem}
 \item remove the dependency of \binderhub{} on Kubernetes
 ...
 \item extend the range of software specification standards that are recognised
   and supported by \repotodocker{}, such as Poetry for Python, renv for R, or Maven for Java.
...
\item The user must interact with Binder environments through a BinderHub
  installation
\item Access to data is not considered, and often challenging or expensive,
...
\item To run the Binder software on systems where Kubernetes is not available (such as
  the Desktop of a scientist). A related use case is ``Binder@home'' (\taskref{applications}{binder-at-home}).
\item To access and use significant amounts of data from inside the software
  environment is not wellsupported. An important use case that cannot be supported due to
  ...
  that dataset. A Binder environment would make this dataset accessible in an
  interactive environment. This is needed for the data publishing
  use cases~(\taskref{applications}{data-publishing}).
\item To use Binder on systems where Docker cannot be used. A use case for this
  ...
  administrative privileges). This prepares the ``Binder@HPC'' use
  case~(\taskref{applications}{binder-at-hpc}).
 \end{compactitem}
